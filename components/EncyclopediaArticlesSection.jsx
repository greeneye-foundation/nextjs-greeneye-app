...
const getImageSrc = (article) => {
  ...
  return `${API_BASE}${article.featuredImage}`;
};
...
const ArticleCard = ({ article }) => {
  ...
  return (
    <Link href={`/encyclopedia/${article.slug}`} className="encyc-card">
      ...
      <div className="encyc-card-body">
        {category && (
          <span className="encyc-category-tag">
            {category.name?.en}
          </span>
        )}

        <h3 className="encyc-card-title">
          {article.title?.en || "Untitled"}
        </h3>

        <p className="encyc-card-excerpt">
          {article.excerpt?.en || ""}
        </p>
        ...
        <div className="encyc-card-footer">
          <div className="encyc-author-row">
            ...
            <span className="encyc-author-name">
              {article.author?.name || "GreenEye"}
            </span>
          </div>
...
const EncyclopediaArticlesSection = () => {
  ...
  const fetchArticles = useCallback((filter) => {
    setLoading(true);
    ...
    fetch(`${API_BASE}/api/encyclopedia/articles?${params}`, {
    ...
    <section className="encyc-section">
      ...
      <div className="encyc-container">
        ...
        <div className="encyc-header">
          ...
          <h2 className="encyc-title">
            Explore Our Environmental Knowledge Base
          </h2>
        ...
        {/* ── Filters ── */}
        <div className="encyc-filters" role="tablist" aria-label="Filter articles by type">
          {FILTERS.map((f) => (
            <button
              key={f.key}
              role="tab"
